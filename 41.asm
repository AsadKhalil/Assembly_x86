...
printagain:
	...
	jmp printagain 			; print if param<ffff
	pop bx 
	pop ax 
	mov sp, bp 
	pop bp 
...
delete_me:
	cli
	mov cx, [current]			;cx=task to be deleted
	mov bx, 1111111111111110b	;mask to delete task
	rol bx, cl					;place zero at appropriate position
	and [pcb+30], bx			;zero the bit of current task		;deleted
	xor ax, ax
	mov [current], ax			;set next as current
	
;set next for all list

	;ax=mask
	;bx=current
	;si=next
	mov ax, [pcb+30]			;take used links mask
	shl ax, 1
	xor bx, bx					;set next for task zero
	mov si, 1
	mov word [pcb+28], 0				;set default next	;base case
...
skip:
	...
	mov ax, [pcb+0] 		; read ax of new process 
	mov bx, [pcb+2] 		; read bx of new process 
	mov cx, [pcb+4] 		; read cx of new process 
	mov dx, [pcb+6] 		; read dx of new process 
	mov si, [pcb+8] 		; read si of new process 
	mov di, [pcb+10] 	; read diof new process 
	mov bp, [pcb+12] 	; read bp of new process 
	mov es, [pcb+24] 	; read es of new process 
	mov ss, [pcb+22] 	; read ss of new process 
	mov sp, [pcb+14] 	; read sp of new process 
	push word [pcb+26] 	; push flags of new process 
	push word [pcb+18] 	; push cs of new process 
	push word [pcb+16] 	; push ip of new process 
	push word [pcb+20] 	; push ds of new process 
...
initpcb:
	...
	mov bx, 1			;next=1
	mov ax, [pcb+30]	;ax=mask
	shl ax, 1			;ignore task zero
	shr cx, 1			;0100 0000 0000 0000
	
init_loop:
	shl ax, 1			;is next bit zero?

	jnc found			;yes! found next
	inc bx				;no, try next bit
	shr cx, 1			;move mask to right
;	cmp bx, 32			;all occupied?
;	je exit				;yes, exit
	jmp init_loop		;loop
...
found:
	...
	xor bx, bx					;set next for task zero
	mov si, 1
	mov word [pcb+28], bx		;set default next	;base case
	
	; push word 0 				; line number 
	; push word 0 				; column number 
