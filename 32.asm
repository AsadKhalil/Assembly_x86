...
previous: dw 0;	index of previous pcb

threads: dw 1 ; number of threads running

counter: dw 0xffff ;max number of times a thread should be executed
temp: dw 0	;a temporary variable
...
printagain: push ax ; line number
	...
	cmp di, [bp - 2]	;checking if the task has been executed specified(counter) number of times
	jnz printagain ;
...
terminate:
	mov word [cs:pcb + bx + 30], 0x0000	;setting dummy variable to 0
	
	dec word [cs: threads]
	
	;otherwise delete at tail
	mov bx, [cs: previous]
	mov cl, 5
	shl bx, cl
	
	mov word [cs:pcb + bx + 28], 0	;updating the next of the previous task in list
...
;search function for finding an unused pcb
;returns numberofpcb in bx
searchpcbs:
	;saving registers
	push ax
	push cx
...
initpcb:push bp
	...
	mov bx, [cs:threads] ; read next available pcb index
	...
	mov word [pcb+bx+ 30], 1	;set dummy
